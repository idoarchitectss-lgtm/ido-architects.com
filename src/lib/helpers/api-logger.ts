 ...
 * 
 * Priority:
 * 1. Client-side: NEXT_PUBLIC_ENABLE_DEV_LOGGING=true/false (browser)
 * 2. Server-side: ENABLE_DEV_LOGGING=true/false
 * 3. Default: true in development, false in production
 * 
 * @example
 * # .env.local - Tắt hoàn toàn dev logging
 * ENABLE_DEV_LOGGING=false
 * NEXT_PUBLIC_ENABLE_DEV_LOGGING=false
 * 
 * # .env.local - Bật dev logging (default)
 * ENABLE_DEV_LOGGING=true
 * NEXT_PUBLIC_ENABLE_DEV_LOGGING=true
 */
const getEnableDevLogging = (): boolean => {
    // Check client-side first (browser)
    if (typeof window !== 'undefined') {
        const clientFlag = process.env.NEXT_PUBLIC_ENABLE_DEV_LOGGING;

...
export default function AdminMediaPage() {
  ...
  const handleTypeChange = (v: string) => { setContentType(v); setPage(1); };
  const handleFolderChange = (v: string) => { setFolder(v); setPage(1); };
          ...
          <div className="flex flex-wrap gap-3">
            {stats.formatDistribution.map((f) => (
              <div key={f.contentType} className="flex items-center gap-2 text-sm">
                <button
                  type="button"
                  onClick={() => { setContentType(contentType === f.contentType ? "all" : f.contentType); setPage(1); }}
